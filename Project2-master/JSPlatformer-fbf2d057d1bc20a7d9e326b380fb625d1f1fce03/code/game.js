...
function Emerald(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.8, 0.8);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Emerald.prototype.type = "emerald";

function Gravball(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.8, 0.8);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 4;
}
Gravball.prototype.type = "gravball";
...
Emerald.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
var wobbleSpeed = 10, wobbleDist = 0.04;

Gravball.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
document.getElementById("myHeader").innerHTML = "Score: " + score;
  ...
  if (type == "lava" && this.status == null) {
    ...
    // If there aren't any emeralds left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "emerald";
         })) {
      this.status = "won";
      this.finishDelay = 1;
      altScore = score;
